...
export default function Home() {
  ...
  const exportJSON = useCallback(() => {
    const totals: { task: string; totalMs: number }[] = Array.from(
      perTaskWeekTotals.entries()
    )
      .sort((a, b) => b[1] - a[1])
  ...
  const exportCSV = useCallback(() => {
    ...
    const entries = Array.from(perTaskWeekTotals.entries()).sort(
      (a, b) => b[1] - a[1]
    );
  ...
  const exportPDF = useCallback(async () => {
    ...
    doc.setFontSize(11);
    doc.text(
      `Week: ${weekStart.toISOString().slice(0, 10)} to ${weekEnd
        .toISOString()
        .slice(0, 10)}`,
      margin,
      y
  ...
  const exportMarkdown = useCallback(() => {
    const lines: string[] = [];
    lines.push(`# TimerTrack Weekly Report`);
    lines.push(``);
    lines.push(
    ...
    // Summary
    const totalHours = Math.floor(weekTotalMs / (1000 * 60 * 60));
    const totalMinutes = Math.floor(
      (weekTotalMs % (1000 * 60 * 60)) / (1000 * 60)
    );
    ...
    lines.push(`- **Sessions:** ${weekSessionsSorted.length}`);
    lines.push(``);

    // Time by task
    lines.push(`## Time by Task`);
    lines.push(``);
    const taskEntries = Array.from(perTaskWeekTotals.entries()).sort(
      (a, b) => b[1] - a[1]
    );
    ...
    // Time by tag
    if (perTagWeekTotals.size > 0) {
      lines.push(`## Time by Tag`);
      lines.push(``);
      const tagEntries = Array.from(perTagWeekTotals.entries()).sort(
        (a, b) => b[1] - a[1]
      );
    ...
    lines.push(`## Sessions`);
    lines.push(``);
    if (weekSessionsSorted.length === 0) {
      lines.push(`No sessions recorded this week.`);
    } else {
      for (const session of weekSessionsSorted) {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `timertrack-week-${weekStart.toISOString().slice(0, 10)}.md`;
    a.click();
    URL.revokeObjectURL(url);
  }, [
    weekStart,
