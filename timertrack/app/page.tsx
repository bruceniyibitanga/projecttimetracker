...
export default function Home() {
  ...
  }>({ title: "", date: formatDateForInput(new Date()), startTime: "09:00", endTime: "10:00", tags: "" });
  ...
  const [exportForm, setExportForm] = useState<{ startDate: string; endDate: string; tagsCsv: string }>(() => {
    const now = new Date();
    const ws = startOfWeek(now);
    const we = endOfWeek(now);
    return { startDate: formatDateForInput(ws), endDate: formatDateForInput(we), tagsCsv: "" };
  });
  ...
  const [pomodoroAutoContinue, setPomodoroAutoContinue] = useState<boolean>(true);
  ...
  useEffect(() => {
    try {
      ...
      if (pomoRaw) {
        try {
          const p = JSON.parse(pomoRaw) as { enabled?: boolean; focus?: number; break?: number; auto?: boolean };
          if (p) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        ...
        JSON.stringify({ enabled: pomodoroEnabled, focus: pomodoroFocusMin, break: pomodoroBreakMin, auto: pomodoroAutoContinue })
      );
    } catch {}
  }, [pomodoroEnabled, pomodoroFocusMin, pomodoroBreakMin, pomodoroAutoContinue]);
  ...
  useEffect(() => {
    const interval = window.setInterval(() => {
      ...
      if (pomodoroPhase === "focus" && isRunning && now >= pomodoroEndMs) {
        handlePause();
      } else if (pomodoroPhase === "break" && !isRunning && now >= pomodoroEndMs) {
  ...
  }, [pomodoroEnabled, pomodoroEndMs, pomodoroPhase, pomodoroAutoContinue, pomodoroFocusMin, handlePause, isRunning, runningTitle, taskTitle]);
  ...
  // Compute totals
  const { todayTotalMs, weekTotalMs, perTaskWeekTotals, perTagWeekTotals } = useMemo(() => {
  ...
  const exportPDF = useCallback(async () => {
    ...
    doc.setFontSize(16);
    doc.text(`TimerTrack Weekly Report`, margin, y);
    y += lineHeight;
    doc.setFontSize(11);
    doc.text(`Week: ${weekStart.toISOString().slice(0, 10)} to ${weekEnd.toISOString().slice(0, 10)}`, margin, y);
    y += lineHeight * 1.5;

    doc.setFont("helvetica", "bold");
    doc.text("Totals by Task", margin, y);
    doc.setFont("helvetica", "normal");
    y += lineHeight;
    const entries = Array.from(perTaskWeekTotals.entries()).sort((a, b) => b[1] - a[1]);
    if (entries.length === 0) {
      doc.text("No tracked time this week.", margin, y);
      y += lineHeight;
    } else {
      for (const [task, totalMs] of entries) {
        doc.text(`${task}: ${formatDuration(totalMs)}`, margin, y);
        y += lineHeight;
        if (y > 760) {
          doc.addPage();
          y = margin;
    ...
    y += lineHeight;
    doc.setFont("helvetica", "bold");
    doc.text("Sessions (This Week)", margin, y);
    doc.setFont("helvetica", "normal");
    y += lineHeight;
    for (const s of weekSessionsSorted) {
      const start = new Date(s.startMs);
      const end = new Date(s.endMs);
      const tags = s.tags && s.tags.length ? ` [${s.tags.join(', ')}]` : "";
      const line = `${formatDuration(s.endMs - s.startMs)}  ${s.title}${tags}`;
    ...
    doc.save(`timertrack-week-${weekStart.toISOString().slice(0, 10)}.pdf`);
  }, [perTaskWeekTotals, weekStart, weekEnd, weekSessionsSorted]);
          ...
          try {
            if ("Notification" in window) {
              ...
              if (Notification.permission === "granted") {
                new Notification("TimerTrack", { body: "Reminder: start tracking your time?" });
                return;
              }
            }
          } catch {
  ...
  return (
    <div className="min-h-screen w-full bg-neutral-50 text-neutral-900">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
            ...
            <div className="hidden sm:flex gap-2">
              ...
              <button
                onClick={() => {
                  ...
                  setEntryForm({ title: taskTitle || "", date: formatDateForInput(new Date()), startTime: "09:00", endTime: "10:00", tags: "" });
        ...
        <section className="mb-8 rounded-lg border border-neutral-200 bg-white p-4">
          <div className="flex flex-col sm:flex-row sm:items-end gap-3">
            <div className="flex-1">
              <label htmlFor="reminder" className="text-sm text-gray-600">Remind me every (minutes)</label>
              <input
                id="reminder"
                type="number"
                min={0}
            ...
            <div className="text-sm text-gray-600">Set to 0 to disable reminders.</div>
          </div>
        </section>
        ...
        <section className="mb-8 rounded-lg border border-neutral-200 bg-white p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={pomodoroEnabled} onChange={(e) => setPomodoroEnabled(e.target.checked)} /> Enable
            </label>
          </div>
          <div className="grid gap-3 sm:grid-cols-4 sm:items-end">
            <div>
              <label className="text-sm text-gray-600">Focus minutes</label>
              <input type="number" min={1} value={pomodoroFocusMin} onChange={(e) => setPomodoroFocusMin(Math.max(1, Number(e.target.value) || 1))} className="mt-1 w-full rounded-md border border-neutral-300 bg-white px-3 py-2 focus:outline-none focus:ring-2 focus:ring-neutral-900" />
            </div>
            <div>
              <label className="text-sm text-gray-600">Break minutes</label>
              <input type="number" min={1} value={pomodoroBreakMin} onChange={(e) => setPomodoroBreakMin(Math.max(1, Number(e.target.value) || 1))} className="mt-1 w-full rounded-md border border-neutral-300 bg-white px-3 py-2 focus:outline-none focus:ring-2 focus:ring-neutral-900" />
            </div>
            <div className="flex items-center gap-2">
              <label className="text-sm text-gray-600">Auto-continue</label>
              <input type="checkbox" checked={pomodoroAutoContinue} onChange={(e) => setPomodoroAutoContinue(e.target.checked)} />
            </div>
            <div className="text-sm text-gray-600">
              {pomodoroEnabled && (
                <div>
                  ...
                  {pomodoroEndMs && (
                    <span className="ml-2">({formatDuration(Math.max(0, pomodoroEndMs - Date.now()))} left)</span>
                  )}
                </div>
              )}
            </div>
        ...
        <section className="mb-8">
          <h2 className="mb-3 text-lg font-semibold">This week by tag</h2>
          {perTagWeekTotals.size === 0 ? (
            <div className="text-sm text-gray-600">No tags recorded this week.</div>
          ) : (
            <div className="divide-y divide-neutral-200 rounded-lg border border-neutral-200 bg-white">
                ...
                .map(([tag, ms]) => (
                  <div key={tag} className="flex items-center justify-between px-4 py-3">
                    <div className="flex min-w-0 items-center gap-3 pr-4">
                      <span className="inline-block h-3 w-3 rounded" style={{ backgroundColor: tagColors[tag] || "#f3f4f6" }} />
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <input
                        ...
                        onChange={(e) => setTagColors((prev) => ({ ...prev, [tag]: e.target.value }))}
        ...
        <section className="mb-8">
          <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <h2 className="text-lg font-semibold">This week sessions</h2>
            <div className="flex flex-wrap items-center gap-2">
              <button onClick={() => setViewMode("list")} className={`rounded border px-2 py-1 text-xs ${viewMode === "list" ? "bg-neutral-900 text-white border-neutral-900" : "border-neutral-300 text-neutral-700"}`}>List</button>
              <button onClick={() => setViewMode("calendar")} className={`rounded border px-2 py-1 text-xs ${viewMode === "calendar" ? "bg-neutral-900 text-white border-neutral-900" : "border-neutral-300 text-neutral-700"}`}>Calendar</button>
              <button onClick={() => setShowExportModal(true)} className="rounded border border-neutral-300 px-2 py-1 text-xs text-neutral-700 hover:bg-neutral-100">Export…</button>
              <button onClick={() => setShowSettingsModal(true)} className="rounded border border-neutral-300 px-2 py-1 text-xs text-neutral-700 hover:bg-neutral-100">Settings</button>
            </div>
          </div>
          {viewMode === "list" && (
            <>
                          ...
                          <div className="min-w-0 pr-2">
                            ...
                            {s.tags && s.tags.length > 0 && (
                              <div className="mt-1 flex flex-wrap gap-1">
                                {s.tags.map((t) => (
                                  <span key={t} className="rounded px-2 py-0.5 text-[10px] text-neutral-800" style={{ backgroundColor: tagColors[t] || "#f3f4f6" }}>#{t}</span>
                                ))}
                              </div>
                            )}
                          </div>
                          <div className="flex items-center gap-2 shrink-0">
                            <div className="font-mono tabular-nums mr-2">{formatDuration(duration)}</div>
                            <button
                              onClick={() => {
                                setEditingSession(s);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-7 gap-3">
              {Array.from({ length: 7 }).map((_, idx) => {
                ...
                const daySessions = sessions
                  .filter((s) => Math.max(s.startMs, dayStart) <= Math.min(s.endMs, dayEnd))
                  .sort((a, b) => a.startMs - b.startMs);
                const totalMs = daySessions.reduce((acc, s) => acc + clampToRangeOverlap(s.startMs, s.endMs, dayStart, dayEnd), 0);
                return (
                  <div key={idx} className="flex h-64 flex-col overflow-hidden rounded-lg border border-neutral-200 bg-white">
                    <div className="flex items-center justify-between border-b border-neutral-100 px-3 py-2">
                      <div className="truncate text-sm font-medium">{day.toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" })}</div>
                      <div className="text-xs font-mono tabular-nums text-gray-600">{formatDuration(totalMs)}</div>
                    </div>
                        ...
                        <ul className="space-y-2">
                          {daySessions.map((s) => (
                            <li key={s.id} className="text-xs">
                              <div className="truncate font-medium break-words">{s.title}</div>
                              <div className="text-[10px] text-gray-600">{new Date(s.startMs).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })} – {new Date(s.endMs).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })} • {formatDuration(s.endMs - s.startMs)}</div>
                              {s.tags && s.tags.length > 0 && (
                                <div className="mt-1 flex flex-wrap gap-1">
                                  {s.tags.map((t) => (
                                    <span key={t} className="rounded px-1.5 py-0.5 text-[9px]" style={{ backgroundColor: tagColors[t] || "#f3f4f6" }}>#{t}</span>
                                  ))}
                                </div>
                              )}
                            </li>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
            <div className="w-full max-w-lg rounded-lg bg-white p-5 shadow-lg">
              <div className="mb-4 flex items-center justify-between">
                <div className="text-lg font-semibold">{editingSession ? "Edit Entry" : "Add Entry"}</div>
                <button onClick={() => setShowEntryModal(false)} className="text-sm text-neutral-600 hover:text-neutral-900">Close</button>
              </div>
              <div className="grid gap-3">
                ...
                <div className="mt-2 flex justify-end gap-2">
                  ...
                  <button
                    onClick={() => {
                      const start = parseDateTimeLocal(entryForm.date, entryForm.startTime);
                      const end = parseDateTimeLocal(entryForm.date, entryForm.endTime);
                        ...
                        setSessions((prev) =>
                          prev.map((it) =>
                            it.id === editingSession.id
                              ? { ...it, title: entryForm.title.trim() || "Untitled Task", startMs: start.getTime(), endMs: end.getTime(), tags }
                              : it
                          )
                        );
                      } else {
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-2 sm:px-4">
            <div className="w-full max-w-lg rounded-lg bg-white p-4 sm:p-5 shadow-lg">
              <div className="mb-4 flex items-center justify-between">
                ...
                <button onClick={() => setShowExportModal(false)} className="text-sm text-neutral-600 hover:text-neutral-900">Close</button>
              </div>
              <div className="grid gap-3 grid-cols-1 sm:grid-cols-3 sm:items-end sm:gap-3">
                <div className="grid gap-1">
                  <label className="text-sm text-gray-600">Start date</label>
                  <input type="date" value={exportForm.startDate} onChange={(e) => setExportForm((f) => ({ ...f, startDate: e.target.value }))} className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 focus:outline-none focus:ring-2 focus:ring-neutral-900" />
                </div>
                <div className="grid gap-1">
                  <label className="text-sm text-gray-600">End date</label>
                  <input type="date" value={exportForm.endDate} onChange={(e) => setExportForm((f) => ({ ...f, endDate: e.target.value }))} className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 focus:outline-none focus:ring-2 focus:ring-neutral-900" />
                </div>
                <div className="grid gap-1 sm:col-span-1">
                  <label className="text-sm text-gray-600">Tags filter</label>
                  <input type="text" placeholder="comma-separated, optional" value={exportForm.tagsCsv} onChange={(e) => setExportForm((f) => ({ ...f, tagsCsv: e.target.value }))} className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 focus:outline-none focus:ring-2 focus:ring-neutral-900" />
                </div>
              </div>
              <div className="mt-4 flex flex-wrap justify-end gap-2">
                <button onClick={() => handleExportFiltered("csv", exportForm, sessions)} className="min-w-[90px] rounded-md border border-neutral-300 px-3 py-2 text-sm text-neutral-700 hover:bg-neutral-100">CSV</button>
                <button onClick={() => handleExportFiltered("json", exportForm, sessions)} className="min-w-[90px] rounded-md border border-neutral-300 px-3 py-2 text-sm text-neutral-700 hover:bg-neutral-100">JSON</button>
                <button onClick={() => handleExportFiltered("pdf", exportForm, sessions)} className="min-w-[90px] rounded-md bg-neutral-900 px-3 py-2 text-sm font-medium text-white hover:bg-neutral-800">PDF</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-2 sm:px-4">
            <div className="w-full max-w-lg rounded-lg bg-white p-4 sm:p-5 shadow-lg">
              <div className="mb-4 flex items-center justify-between">
                ...
                <button onClick={() => setShowSettingsModal(false)} className="text-sm text-neutral-600 hover:text-neutral-900">Close</button>
              </div>
              <div>
                ...
                <div className="grid grid-cols-1 gap-2">
                  {Array.from(new Set(sessions.flatMap((s) => s.tags || []))).length === 0 ? (
                    ...
                    Array.from(new Set(sessions.flatMap((s) => s.tags || []))).sort().map((tag) => (
                      <div key={tag} className="flex flex-wrap items-center justify-between gap-3 rounded border border-neutral-200 p-2">
                        <div className="text-sm">#{tag}</div>
                        <input type="color" value={tagColors[tag] || "#f3f4f6"} onChange={(e) => setTagColors((prev) => ({ ...prev, [tag]: e.target.value }))} />
                      </div>
                    ))
                  )}
                </div>
        ...
        <div className="fixed inset-x-0 bottom-0 z-40 border-t border-neutral-200 bg-white/95 p-3 backdrop-blur sm:hidden">
          <div className="mx-auto flex max-w-3xl items-center justify-between">
            <div className="truncate pr-4 text-sm">
              {isRunning ? (runningTitle || taskTitle || "Untitled Task") : (taskTitle || "No task")}
            </div>
            <button onClick={handleToggle} className={`rounded-md px-4 py-2 text-sm font-medium text-white ${isRunning ? "bg-amber-600" : "bg-neutral-900"}`}>
              {isRunning ? "Pause" : "Start"}
            </button>
          </div>
        </div>
...
function handleExportFiltered(format: ExportFormat, form: { startDate: string; endDate: string; tagsCsv: string }, sessions: TimerSession[]) {
  ...
  const sessionsInRange = sessions
    ...
    .filter((s) => (tags.length === 0 ? true : (s.tags || []).some((t) => tags.includes(t))));

  if (format === "json") {
    const payload = { startISO: new Date(startMs).toISOString(), endISO: new Date(endMs).toISOString(), sessions: sessionsInRange };
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
  ...
  if (format === "csv") {
    ...
    for (const s of sessionsInRange) {
      ...
      rows.push(`${escapeCsv(s.title)},${new Date(s.startMs).toISOString()},${new Date(s.endMs).toISOString()},${dur},${formatDuration(dur)},${escapeCsv((s.tags || []).join(";"))}`);
    }
    const csv = rows.join("\n");
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
  ...
  if (format === "pdf") {
    import("jspdf").then(({ jsPDF }) => {
      ...
      y += lineHeight;
      doc.setFontSize(11);
      doc.text(`Range: ${form.startDate} → ${form.endDate}${tags.length ? ` | Tags: ${tags.join(', ')}` : ''}`, margin, y);
      y += lineHeight * 1.5;
      ...
      if (sessionsInRange.length === 0) {
        ...
        for (const s of sessionsInRange) {
          const when = `${new Date(s.startMs).toLocaleString()} → ${new Date(s.endMs).toLocaleString()}`;
          doc.text(`${formatDuration(s.endMs - s.startMs)}  ${s.title}`, margin, y);
          y += lineHeight;
          doc.setFontSize(10);
          doc.setTextColor(100);
          doc.text(when, margin + 12, y);
